...
describe("TodoApp", () => {
  ...
  describe('Delete all items',  () => {
    it('should delete all items when called', async () => {
      userEvent.type(newItemTextField, "Write one todo");
      userEvent.click(newItemButton);
      userEvent.type(newItemTextField, "Write two todo");
      userEvent.click(newItemButton);
      let items = await renderedComponent.findAllByTestId('todoItem');
      expect(items.length).toBe(2);
      userEvent.click(deleteAllButton);
      await expect(renderedComponent.findAllByRole('todoItem')).rejects.toThrow();
    })
  });
});
...
describe("TodoItemComponent", () => {
  ...
  it('Should change state when liked', async () => {
    let likeButton = await renderedComponent.findByTestId('likedButton');
    expect(likeButton).toHaveAttribute('aria-label', 'like');
    userEvent.click(likeButton);
    likeButton = await renderedComponent.findByTestId('likedButton');
